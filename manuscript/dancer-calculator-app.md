...
is still using the port, but it currently does not provide service. If you type `jobs` in the terminal that this happend
...
You can kill this job by typing `kill %1` or by moving it to the forground using `fg` and then killing it using `Ctrl-C`.
